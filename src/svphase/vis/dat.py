...
def vis_hic_cut(contig, hic_dat, cuts_fpath):
...
def vis_hic(contig, hic_dat, out_fpath=None):
	dat_p = VisualizeDat(hic_dat)
...
def vis_hic_zoom(contig, hic_dat, out_fpath=None, resolution=200, window_size=200, windows=50):
...
if __name__ == '__main__':
	...
	parser = argparse.ArgumentParser(description='Visualization of HiC DAT file')
	parser.add_argument('contig', help='chromosome')
	parser.add_argument('dat', help='HiC reads in DAT format')
	parser.add_argument('-r', dest='resolution', type=int, default=200, help='Resolution of bin sizes')
	parser.add_argument('-wn', dest='windows', type=int, default=100, help='# of windows to average')
	parser.add_argument('-ws', dest='window_size', type=int, default=200, help='width of window in units of resolution')
	parser.add_argument('-o', dest='out_fpath', default=None, help='Save image with matplotlib')
	parser.add_argument('-c', dest='cut',  default=None, help='Fpath with cutsites')	
	parser.add_argument('-z','--zoom', dest='zoom', action='store_true', default=False, help='Zoomed view')	

	args = parser.parse_args()

	if not args.cut is None:
		vis_hic_cut(args.contig, args.dat)
	elif args.zoom:
		vis_hic_zoom(args.contig, args.dat, resolution=args.resolution, windows=args.windows, window_size=args.window_size, out_fpath=args.out_fpath)
	else:
		vis_hic(args.contig, args.dat, out_fpath=args.out_fpath)
