...
class Data(object):
	...
	def get_nonzero(self, combines=None):
		cols = range(0, len(self.loci))
		wgs = self._nonzero(self._rm_break_info(self.wgs).loc[:, cols].astype(float))
		wgs_allele = self._nonzero(self._rm_break_info(self.wgs_allele).loc[:, cols].astype(float))
		hic_allele = self._nonzero(self._rm_break_info(self.hic_allele).loc[:, cols].astype(float))
		hic_allele_ncut = self._nonzero(self._rm_break_info(self.hic_allele_ncut).loc[:, cols].fillna(0).astype(float))

		if combines == 'hic_only':
			idx = na.logical_or(hic_allele, hic_allele_ncut)
		elif combines == 'wgs_only':
			idx = wgs_allele.values
		elif combines == 'only':
			idx = wgs.values
		else:
			idx = na.logical_or(na.logical_or(hic_allele, hic_allele_ncut), wgs_allele)
		# print self._rm_break_info(self.wgs_allele)[cols].shape, idx.shape
		return idx
	
	def get_features(self, combines=None):
		if self.call_once_flag:
			raise Exception('Cannot call get feats more than once')
		self._add_data_type()
		...
		if combines == 'hic_only':
			c = [self.wgs, self.hic_allele, self.hic_allele_ncut]
		elif combines == 'wgs_only':
			c = [self.wgs, self.wgs_allele]
		elif combines == 'only':
			c = [self.wgs, ]
		else:
			c = [self.wgs, self.wgs_allele, self.hic_allele, self.hic_allele_ncut]

		features = pd.concat(map(self._rm_break_info, c),
		...
		cols = [i for i in self.row_index_header if i in cols_set] + range(0, len(self.loci))
		features = features[cols]
		# print "Cols"
		# print feats
		self.call_once_flag = True
		return features.rename(index=dict(zip(features.index, count(0))))		

	def get_simple_sum_features(self, combines=None):
		self._add_data_type()
		self._rm_wrong_orients()
		self._rm_dist()
		self._rm_hic_arounds()
		...
		# print "Cols"
		# print feats
		features = features.rename(index=dict(zip(features.index, count(0)))) 
		# print feats.shape
		# return feats
		features = features.drop(features.columns.intersection(['orient', 'orient1', 'orient2']), axis=1)
		
		if 'allele' in features.columns:
			rheaders = ['data_type', 'wtype', 'allele']
			wgs_cov, wgs_around = ('wgs', 'cov', ''), ('wgs', 'w.around_w.around', '')
		else:
			rheaders = ['data_type', 'wtype']
			wgs_cov, wgs_around = ('wgs', 'cov'), ('wgs', 'w.around_w.around')

		features[rheaders] = features[rheaders].fillna('')
		g = features.groupby(rheaders)
		
		# print feats.ix[:,:10]
		 
		new_features = pd.DataFrame(g.get_group(wgs_cov).drop(rheaders, axis=1).values, index=pd.Index(['wgs-cov', ], name='data_type'), dtype=float).T
		new_features['wgs-around'] = g.get_group(wgs_around).drop(rheaders, axis=1).values.ravel()
		# new_features = pd.DataFrame([na.zeros(len(feats.columns)-len(rheaders))],index=['wgs-cov',]).T
		for allele in ['A', 'B']:
			for support in [-1, 1]:
				new_features['%d*%s' % (support, allele)] = 0
 
		if 'allele' in features.columns:
			for (dtype, wtype, allele), values in g:
				values = values.drop(rheaders, axis=1).astype(float)
				if dtype != 'wgs':
					# print values.sum(axis=0)
					# print new_features['1*%s'%(allele)]
					if wtype == 'w.around_w.around':
						new_features['-1*%s' % (allele)] += values.sum(axis=0).values 
					else:
						new_features['1*%s' % (allele)] += values.sum(axis=0).values 
		# print g.groups
		# print new_features.shape
		# print new_features
		new_features = new_features.T.reset_index()
